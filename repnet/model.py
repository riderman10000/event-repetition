...
class RepNet(nn.Module):
    """RepNet model."""
    def __init__(self, num_frames: int = 64, temperature: float = 13.544, encoder  = True):
        super().__init__()
    ...
    def period_predictor(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        batch_size, seq_len, _ = x.shape # 1 x 64 x 512 
        torch._assert(seq_len == self.num_frames, f'Expected {self.num_frames} frames, got {seq_len}')
        # Compute temporal self-similarity matrix
        x = torch.cdist(x, x)**2 # N x D x D # 1, 64, 64 
    ...
    def event_period_predictor(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass of the period predictor network from the extracted embeddings. Expected input shape: N x D x C."""
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # test period_predictor _RL 
        embeddings = x 
        period_length, periodicity = self.event_period_predictor(embeddings)
